...
void ScheduleIndexJobs(
    std::string index_id, int32_t partitions, std::string client_id,
    std::shared_ptr<tasker::Driver> driver, std::string src_file,
  ...
  std::shared_ptr<tasker::Jobs> indexing_jobs = std::make_shared<tasker::Jobs>(
      std::make_shared<std::function<void(int32_t, int32_t, std::string)>>(
          [=](int32_t failed_count, int32_t failed_code, std::string msg) {
            ...
            driver->SendToClient(client_id, tasker::GetCommand(tasker::Commands::UPDATE),
                                 "All indexing jobs completed in " + std::to_string(time) + "sec");
            // on all jobs done
      ...
      std::make_shared<std::function<void(std::string, int32_t, std::string, int32_t, int32_t)>>(
          [=](std::string job_id, int32_t code, std::string msg, int32_t failed_jobs,
            ...
            spdlog::info("Index {} completed {}/{}, Failures : {}", index_id, completed_jobs,
                         partitions, failed_jobs);
          }),
  ...
  for (int32_t idx = 0; idx < partitions; idx++) {
    ...
    std::shared_ptr<IndexJob> idx_job =
        std::make_shared<IndexJob>(idx_src, job_id, idx, client_id, driver);
    spdlog::info("Created index job for partition {}", idx + 1);
    indexing_jobs->AddJob(idx_job);
    spdlog::info("Added job for partition {}", idx + 1);
  }
  job_handlers.insert(
      std::pair<std::string, std::shared_ptr<tasker::Jobs>>(jobs_id, indexing_jobs));
  indexing_jobs->Execute();
}

void HandleIndex(
    std::string msg, std::string client_id, std::shared_ptr<tasker::Driver> driver,
    std::shared_ptr<std::function<void(int32_t, std::string, std::string)>> create_index) {
...
void ScheduleMerge(std::string result_id, std::string client_id, int32_t partitions,
  ...
  mrg_job->OnComplete(std::make_shared<std::function<void(std::string, int32_t, std::string)>>(
      [=](std::string job_id, int32_t code, std::string msg) {
        if (code != 0) {
          spdlog::info("Merge job {} has failed.");
          // reporting the error to the client
          driver->SendToClient(client_id, tasker::GetCommand(tasker::Commands::MESSAGE), msg);
        } else {
          driver->SendToClient(client_id, tasker::GetCommand(tasker::Commands::MESSAGE),
                               "Seach command completed.");
        }
      }));

  driver->GetExecutor()->AddJob(mrg_job);
}

void ScheduleSearch(std::string index_id, std::string result_id, std::string client_id,
      ...
      std::make_shared<std::function<void(int32_t, int32_t, std::string)>>(
          [=](int32_t failed_count, int32_t failed_code, std::string msg) {
            ...
            if (failed_count != 0) {
              ...
              driver->SendToClient(client_id, tasker::GetCommand(tasker::Commands::UPDATE),
                                   "Search completed. Doing merge...");
              ScheduleMerge(result_id, client_id, partitions, driver);
            }
            job_handlers.erase(jobs_id);
          }),
      std::make_shared<std::function<void(std::string, int32_t, std::string, int32_t, int32_t)>>(
          [=](std::string job_id, int32_t code, std::string msg, int32_t failed_jobs,
            ...
            spdlog::info("Search completed {}/{}, Failures : {}", completed_jobs, partitions,
                         failed_jobs);
          }),
  ...
  for (size_t i = 0; i < partitions; i++) {
    ...
    std::string input_file =
        get_root() + "/" + result_id + "/mread-" + std::to_string(i + 1) + ".fa";
    ...
    std::shared_ptr<SearchJob> src_job = std::make_shared<SearchJob>(
        input_file, index_file, dest_file, i, job_id, client_id, driver);
...
void ScheduleDispatch(SearchClientCommand& search_command, std::string client_id,
  ...
  std::shared_ptr<tasker::Jobs> dispatch_jobs = std::make_shared<tasker::Jobs>(
      std::make_shared<std::function<void(int32_t, int32_t, std::string)>>(
          [=](int32_t failed_count, int32_t failed_code, std::string msg) {
            ...
            if (failed_count != 0) {
              ...
              for (size_t i = 0; i < partitions; i++) {
                std::string maxinf_f =
                    get_root() + "/" + results_folder + "/maxinf-" + std::to_string(i + 1);
                if (std::filesystem::exists(maxinf_f)) {
                  ...
                  driver->SendToClient(client_id, "Couldn't find maxinf for partition " +
                                                      std::to_string(i + 1) +
                                                      ". Operation aborted.");
                  break;
                }
              ...
              // write to maxinf
              std::ofstream ofs;
              ofs.open(get_root() + "/" + results_folder + "/maxinf",
      ...
      std::make_shared<std::function<void(std::string, int32_t, std::string, int32_t, int32_t)>>(
          [=](std::string job_id, int32_t code, std::string msg, int32_t failed_jobs,
            ...
            spdlog::info("Dispatch completed {}/{}, Failures : {}", completed_jobs, partitions,
                         failed_jobs);
          }),
  ...
  for (size_t i = 0; i < partitions; i++) {
    std::string job_id = gen_random(16);
    std::string msg = "dsp -b 25 -p " + std::to_string(partitions) + " -s " +
                      search_command.GetSrcFile() + " -i " + search_command.GetIndexId() + " -d " +
                      results_folder + " -g " + std::to_string(i);
    spdlog::info("Generated dispatch command {}", msg);
    std::shared_ptr<DispatchJob> dsp_job =
        std::make_shared<DispatchJob>(msg, job_id, client_id, i, driver);
    dispatch_jobs->AddJob(dsp_job);
  }

  spdlog::info("Scheduling dispatch jobs...");
  job_handlers.insert(
      std::pair<std::string, std::shared_ptr<tasker::Jobs>>(jobs_id, dispatch_jobs));
  dispatch_jobs->Execute();
}

void HandleSearch(std::string msg, std::string client_id, std::shared_ptr<tasker::Driver> driver) {
